...
const SOURCE_CHECKED_AT = "2026-07-28";
...
const normalizeTitle = (value) =>
  textOnly(value)
    .replace(/^Rainbow Magic Graphic Novel:\s*/i, "")
...
if (localBooks.length !== 324) throw new Error(`Expected 324 local records, found ${localBooks.length}`);
...
if (officialCardCount !== 299 || officialUniqueTitles.size !== 296) {
...
const unassigned = localBooks.filter((book) => !assigned.has(normalizeTitle(book.title)));
const archiveDefinitions = [
  { id: "beginner-readers", name: "Rainbow Magic Beginner Readers", prefix: "Rainbow Magic Beginner Reader:" },
  { id: "early-readers", name: "Rainbow Magic Early Readers", prefix: "Rainbow Magic Early Reader:" },
  { id: "reference-books", name: "Rainbow Magic Reference Books", prefix: "" },
];

for (const definition of archiveDefinitions) {
  const books = definition.prefix
    ? unassigned.filter((book) => book.title.startsWith(definition.prefix))
    : unassigned.filter((book) => !book.title.startsWith("Rainbow Magic Beginner Reader:") && !book.title.startsWith("Rainbow Magic Early Reader:"));
  catalogGroups.push({ id: definition.id, name: definition.name, sourceKind: "official-archive", books });
  for (const book of books) assigned.add(normalizeTitle(book.title));
}
...
if (catalogGroups.length !== 41) throw new Error(`Expected 41 checklist groups, found ${catalogGroups.length}`);
